...
LIMIT 10 ;
-- Donner la liste des 10 produits les moins chers
...
SELECT  SUM(MontantTotal) TotalVente
FROM Ventes;
...
SELECT  AVG(MontantTotal) MoyenneDesVentes
FROM Ventes;
...
SELECT MIN(PrixUnitaire) ProduitMoinsCher,  MAX(PrixUnitaire) ProduitPlusCher
FROM Produits; 
...
SELECT SUM(DISTINCT(MontantTotal)) CA
FROM Ventes;
...
SELECT AVG(DISTINCT(MontantTotal)) MoyenneCA
FROM Ventes;
/*==================================================================================
...
SELECT EmployeID, SUM(MontantTotal) CA
FROM Ventes
GROUP BY EmployeID; 
...
SELECT EmployeID, COUNT(VenteID) NbVentes
FROM Ventes
GROUP BY EmployeID; 

-- Exemple pratique: Déterminer la somme des ventes par année.

SELECT  YEAR (DateVente) Annee, SUM(MontantTotal) CAParAN
FROM Ventes
GROUP BY Annee; 
...
SELECT EmployeID,YEAR(DateVente) Annee, AVG(MontantTotal) MoyenneVentes
FROM Ventes
...
SELECT EmployeID, AVG(MontantTotal) MoyenneDesVentes
FROM Ventes 
GROUP BY EmployeID
HAVING MoyenneDesVentes >1000; 
